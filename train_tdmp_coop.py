...
from clip.coop import CustomCLIP, ZSCLIPModel
import os
import time
import logging
...
from model.tpt import TPT
from model.grad_trans import GradientTransform2
...
def get_arguments():
    ...
    parser.add_argument('--backbone', default='ViT-B/16', type=str)  # ViT-B/32
    ...
    parser.add_argument('--inner_lr', default=3e-3, type=float)
...
def main():
    ...
    # Load clip
    clip_model, _, _ = clip.load(args.backbone, device=device)
    ...
    model = CustomCLIP(clip=clip_model, classnames=meta_dataset.classnames,
    ...
    metalearner = MetaLearner(args=args, model=model, optimizer_model=optimizer_model,
                              lr_scheduler_model=lr_scheduler_model, device=device)
    ...
    with torch.no_grad():
        ...
        model.prompt_learner.ctx_init_state = meta_prompt_weight["ctx"].clone().to(device)
    ...
    target_dataloader = DataLoader(target_dataset, batch_size=1, shuffle=True,
                                   num_workers=16, pin_memory=True)#1
